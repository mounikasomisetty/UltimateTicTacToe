...
from team21Old import Player21Old
...
def decide_winner_and_get_message(player,status, message,pl1,pl2):
	if player == 'P1' and status == 'L':
		pl1.updateFile(pl1.scale(0));
		pl2.updateFile(pl2.scale(4));
		return ('P2',message)
	elif player == 'P1' and status == 'W':
		pl1.updateFile(pl1.scale(4));
		pl2.updateFile(pl2.scale(0));
		return ('P1',message)
		...
		pl1.updateFile(pl1.scale(2));
		pl2.updateFile(pl2.scale(2));
		return ('NONE','DRAW')
	return
...
def simulate(obj1,obj2):
	...
	while(1):
		...
		try:
			...
			WINNER, MESSAGE = decide_winner_and_get_message('P1', 'L',   'TIMED OUT',pl1,pl2)
			break
		signal.alarm(0)
		...
		if not (verification_fails_board(game_board, temp_board_state) and verification_fails_block(block_stat, temp_block_stat)):
			...
			WINNER, MESSAGE = decide_winner_and_get_message('P1', 'L',   'MODIFIED CONTENTS OF LISTS',pl1,pl2)
			break
		...
		if not check_valid_move(game_board, block_stat,ret_move_pl1, old_move):
			...
			WINNER, MESSAGE = decide_winner_and_get_message('P1', 'L',   'MADE AN INVALID MOVE',pl1,pl2)
			break
		...
		if gamestatus == True:
			#print_lists(game_board, block_stat)
			WINNER, MESSAGE = decide_winner_and_get_message('P1', mesg,  'COMPLETE',pl1,pl2)
		...
		try:
			...
			WINNER, MESSAGE = decide_winner_and_get_message('P2', 'L',   'TIMED OUT',pl1,pl2)
			break
		signal.alarm(0)

                if not (verification_fails_board(game_board, temp_board_state) and verification_fails_block(block_stat, temp_block_stat)):
			WINNER, MESSAGE = decide_winner_and_get_message('P2', 'L',   'MODIFIED CONTENTS OF LISTS',pl1,pl2)
			break
			
                if not check_valid_move(game_board, block_stat,ret_move_pl2, old_move):
			WINNER, MESSAGE = decide_winner_and_get_message('P2', 'L',   'MADE AN INVALID MOVE',pl1,pl2)
			break
		...
		gamestatus, mesg =  terminal_state_reached(game_board, block_stat)
                if gamestatus == True:
			#print_lists(game_board, block_stat)
                        WINNER, MESSAGE = decide_winner_and_get_message('P2', mesg,  'COMPLETE', pl1,pl2)
                        break
...
if __name__ == '__main__':
	...
	option = sys.argv[1]	
	if option == '1':
		print "Player 21 vs. Player21Old"
		obj1 = Player21()
		obj2 = Player21Old()

	elif option == '2':
		print "Random vs Player21"
		obj1 = Player1()
		obj2 = Player21()
	elif option == '3':
		print "Random vs Player21Old"
		obj1 = Player1()
		obj2 = Player21Old()
